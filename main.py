...
# Make project-local packages importable even when the active interpreter was
# started with a restricted or nonstandard module search path (common in WSL
# virtual environments launched from mounted Windows directories).
# ``Path.resolve`` may fail transiently on WSL/DrvFs mounted paths when the
# interpreter receives a relative ``__file__``.  ``absolute`` is sufficient
# here and does not require resolving every mount-point component.
...
def run(
    ...
    *,
    ...
    num_fittest = int(SEARCH_SETTINGS["NUM_FIT_CIPHERS"])
    if init:
